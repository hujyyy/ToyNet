...
public class SoundCtrl_mapping{
 ...
 public int decoding(int packlength,byte[] tmpbuffer,int start_index,byte[] refer0,byte[] refer1){
    ...
    fft_test ft = new fft_test();
     ...
     for(int i=start_index;i<stop_index;i=i+samples_per_bit){
       ft.getfreq(tmpbuffer,i,16,winsize,freq);
       ...
       // else output.append("0");
       if(freq[2]>500) tri_result += "00";
       if(freq[6]>500) tri_result += "01";
       if(freq[10]>500) tri_result += "10";
       if(freq[15]>500) tri_result += "11";
  ...
  public void get_ref(byte[] refer0,byte[] refer1,int samples_per_bit,double sample_rate,int scale){
    ...
    public void FSK(int num_pack,int samples_per_bit,int rep_times){
      int scale = 50;
      int fft_winsize = 32;
        try {
            ...
            for (int i=0;i<rep_times;i++) tmpstr += result ;//transfer n times
            result = tmpstr;
            int sample_rate = 44000;
            ...
            double frequencyOfSignal2 = 44000/fft_winsize*6;
            double frequencyOfSignal5 = 44000/fft_winsize*10;
            double frequencyOfSignal6 = 44000/fft_winsize*15;
            ...
            double samplingInterval6 = (double) (sample_rate/frequencyOfSignal6);
            ...
            for(int j = 0;j<num_pack;j++){
              ...
              for(int i = 0;i<bits_per_pack;i+=2){
                if (inputarray [input_index] ==48 && inputarray [input_index+1] ==48){
                  //System.out.print(index+"\n");
                ...
                if(inputarray [input_index] ==48 && inputarray [input_index+1] ==49){
                  for (int k = 0;k<samples_per_bit;k++){
                ...
                if(inputarray [input_index] ==49 && inputarray [input_index+1] ==48){
                  for (int k = 0;k<samples_per_bit;k++){
                    double angle5 = (2.0 * Math.PI * k) / samplingInterval5;

                    soundarray[index++] += (byte) (scale * Math.sin(angle5));

                  }
                }

                if(inputarray [input_index] ==49 && inputarray [input_index+1] ==49){
                  for (int k = 0;k<samples_per_bit;k++){
                    double angle6 = (2.0 * Math.PI * k) / samplingInterval6;

                    soundarray[index++] += (byte) (scale * Math.sin(angle6));
  ...
  public static void main(String[] args) throws IOException,InterruptedException{
    ...
    while (ProgramRun) {
      ...
      SoundCtrl_mapping sc = new SoundCtrl_mapping();
      ...
      if (userChoice == 1) {
        ...
        sc.running=false;
          ...
          sc.FSK(num_pack,samples_per_bit,rep_times);
